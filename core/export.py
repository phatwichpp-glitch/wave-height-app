...
def build_excel(results: List[Dict[str, Any]], selected_epoch_idx: int = 0) -> bytes:
    ...
    ws1 = wb.active
    ws1.title = "Summary"
    ws1.append(["Method", "Mean (m)", "Max (m)", "Min (m)", "Std Dev (m)"])
    style_header_row(ws1, 1)

    methods = {
        "Integration (Hs_rect)":    [r.get("Hs_rect",  np.nan) for r in results],
        "Spectral (Hs_spec)":       [r.get("Hs_spec",  np.nan) for r in results],
        "Zero-crossing (Hs_zc)":    [r.get("Hs_zc",    np.nan) for r in results],
    }
    ...
    for r in results:
        ...
        ws2.append([
            fmt(r.get("epoch_start")), fmt(r.get("epoch_end")),
            fmt(r.get("Hs_rect")),    fmt(r.get("Hs_spec")),
            fmt(r.get("Hs_zc")),      fmt(r.get("Tp")),
            fmt(r.get("Tm02")),       fmt(r.get("H_max")),
            fmt(r.get("T_mean")),     r.get("wave_count", 0),
        ])
    auto_width(ws2)

    # ── Sheet 3: Spectrum ─────────────────────────────────────────────────────
    ws3 = wb.create_sheet("Spectrum")
    ws3.append(["Frequency (Hz)", "Spectral Density (m^2/Hz)"])
...
_PDF_FONT = "Helvetica"   # safe default — no Thai font needed
...
def _make_spectrum_fig(results: List[Dict[str, Any]], epoch_idx: int) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(9, 3.5), facecolor="white")
    ax.set_facecolor("white")
...
def _make_scatter_fig(results: List[Dict[str, Any]]) -> plt.Figure:
    ...
    for ax, y, ylabel, color, title in [
        (axes[0], Hs_spec, "Hs_spec (m)", "red",   "Hs_rect vs Hs_spec"),
        (axes[1], Hs_zc,   "Hs_zc (m)",  "green", "Hs_rect vs Hs_zc"),
    ]:
...
def build_pdf(results: List[Dict[str, Any]],
    ...
    normal_style = ParagraphStyle("Normal2", fontName=F, fontSize=11, leading=16)
    title_style  = ParagraphStyle("Title2",  fontName=F, fontSize=18, leading=24,
                                  alignment=1, spaceAfter=12)
    h2_style     = ParagraphStyle("H2",      fontName=F, fontSize=14, leading=20,
    ...
    tbl_info.setStyle(tbl_style)
    story.append(tbl_info)
    story.append(Spacer(1, 0.5*cm))

    story.append(Paragraph("Statistical Summary", h2_style))
    methods_map = {
        "Integration (Hs_rect)": [r.get("Hs_rect", np.nan) for r in results],
        "Spectral (Hs_spec)":    [r.get("Hs_spec", np.nan) for r in results],
        "Zero-crossing (Hs_zc)": [r.get("Hs_zc",  np.nan) for r in results],
    }
    stat_rows = [["Method", "Mean (m)", "Max (m)", "Min (m)", "SD (m)"]]
    ...
    tbl_stat.setStyle(tbl_style)
    story.append(tbl_stat)
    story.append(PageBreak())

    # ── Page 2: Hs Time Series ─────────────────────────────────────────────────
    story.append(Paragraph("Significant Wave Height — Time Series", h2_style))
    ...
    for i, r in enumerate(results):
        ...
        rows.append([
            str(i + 1),
            fv(r.get("Hs_rect")), fv(r.get("Hs_spec")),
            fv(r.get("Hs_zc")),   fv(r.get("Tp"), 2),
            fv(r.get("Tm02"), 2), fv(r.get("H_max")),
            str(r.get("wave_count", 0)),
        ])
    col_w = [1.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2*cm]
    tbl_epoch = Table(rows, colWidths=col_w, repeatRows=1)
    tbl_epoch.setStyle(tbl_style)
