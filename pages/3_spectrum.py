...
if f_arr is not None and Pxx_arr is not None and len(f_arr) > 0:
    ...
    with col_stats:
        ...
        st.metric("fp (peak frequency)", fmt(fp, 4, "Hz"))
        ...
        st.metric("Wave count", str(wc))

else:
    st.warning(f"No spectral data available for Epoch {epoch_idx + 1}")
...
if disp is not None and len(disp) >= 64:
    try:
        ...
        if Sxx_band.size > 0 and np.any(Sxx_band > 0):
            ...
            cbar = fig2.colorbar(pcm, ax=ax2, label="Power (m²/Hz)")
            ...
            st.info("Signal energy in the wave frequency band is too low")

    except Exception as e:
        st.warning(f"Could not generate spectrogram: {e}")
else:
    st.info("At least 64 displacement samples are required for a spectrogram")

# ─── Multi-Epoch Spectrum Overlay ─────────────────────────────────────────────
st.markdown("### 📈 Compare Spectra Across Epochs")
...
if len(selected_epochs) > 0:
    ...
    ax3.set_title("Multi-Epoch Spectrum Comparison", fontsize=11)
