...
def safe_rmse(a, b):
    ...
    return float(np.sqrt(np.mean((a[mask] - b[mask]) ** 2))) if mask.sum() >= 2 else np.nan
...
ax1.plot(epochs, Hs_rect, "b-o", markersize=3, linewidth=1, label="Hs_rect (Integration)")
...
ax1.set_title("Significant Wave Height — Time Series", fontsize=11)
...
for ax, (x_arr, y_arr, xlabel, ylabel, color, title) in zip(axes, scatter_pairs):
    ...
    if mask.sum() > 1:
        ...
        ax.scatter(x_m, y_m, c=color, alpha=0.6, s=30,
                   edgecolors="white", linewidths=0.3)
        lo = min(x_m.min(), y_m.min()) * 0.95
        hi = max(x_m.max(), y_m.max()) * 1.05
...
# ── 3. Metrics Table ──────────────────────────────────────────────────────────
st.markdown("### 📋 Statistics per Method")
...
for label, arr in [("Hs_rect", Hs_rect), ("Hs_spec", Hs_spec), ("Hs_zc", Hs_zc)]:
    ...
    if len(valid) == 0:
        rows.append({"Method": label, "Mean": "-", "Max": "-", "Min": "-",
                     "SD": "-", "RMSE vs Hs_spec": "-"})
        continue
...
if mask_both.sum() > 1:
    ...
    ax3.set_title("Bias Distribution: Hs_rect vs Hs_spec", fontsize=11)
    ...
    st.info("Insufficient data for bias histogram.")
...
if mask_hmax.any() or mask_hszc.any():
    ...
    if mask_hszc.any():
        ax5.fill_between(epochs[mask_hszc], Hs_zc[mask_hszc], alpha=0.3, color="green")
